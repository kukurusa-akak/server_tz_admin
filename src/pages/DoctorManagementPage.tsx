...
import { getBranches, getDoctors, deleteDoctor, type Doctor, type Branch } from "@/lib/api";
...
export function DoctorManagementPage() {
    ...
    const fetchDoctorsAndBranches = async () => {
      try {
        setIsLoading(true);
        const [doctorsData, branchesData] = await Promise.all([getDoctors(), getBranches()]);
    ...
    useEffect(() => { fetchDoctorsAndBranches(); }, []);
  
    const filteredDoctors = useMemo(() => {
        return doctors.filter(doc => {
            const searchTermLower = searchTerm.toLowerCase();
            const nameMatch = doc.name.toLowerCase().includes(searchTermLower);
            const employeeIdMatch = doc.employeeId?.toLowerCase().includes(searchTermLower);
            const branchMatch = selectedBranches.length === 0 || selectedBranches.includes(doc.branchId);
            const statusMatch = selectedStatuses.length === 0 || selectedStatuses.includes(doc.status);

            const hireDate = doc.hireDate ? new Date(doc.hireDate) : null;
            ...
            const resignationDate = doc.resignationDate ? new Date(doc.resignationDate) : null;
    ...
    }, [doctors, searchTerm, selectedBranches, selectedStatuses, hireDateRange, resignationDateRange]);
    ...
    const handleDelete = async (id: number) => {
      try { await deleteDoctor(id); fetchDoctorsAndBranches(); }
    ...
    return (
      <div className="p-6 sm:p-10 space-y-6">
        <Card>
          ...
          <CardContent>
            <div className="text-sm text-muted-foreground mb-4">{filteredDoctors.length}명의 의료진이 검색되었습니다.</div>
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {isLoading ? (
                  ...
                  filteredDoctors.map((doc) => (
                    <TableRow key={doc.id}>
                      ...
                      <TableCell className="font-medium flex items-center gap-3"><img src={doc.imageUrls?.[0] || 'https://via.placeholder.com/40'} alt={doc.name} className="w-10 h-10 rounded-full object-cover" /><div>{doc.name}<p className="text-xs text-muted-foreground">{doc.position}</p></div></TableCell>
                      <TableCell>{doc.branch?.name || 'N/A'}</TableCell>
                      <TableCell><span className={`px-2 py-1 text-xs font-semibold rounded-full ${doc.status === 'ACTIVE' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{doc.status === 'ACTIVE' ? '재직중' : doc.status === 'ON_LEAVE' ? '휴직' : '퇴사'}</span></TableCell>
                      <TableCell>{doc.hireDate ? new Date(doc.hireDate).toLocaleDateString() : 'N/A'}</TableCell>
                      <TableCell>{doc.resignationDate ? new Date(doc.resignationDate).toLocaleDateString() : 'N/A'}</TableCell>
                      <TableCell className="text-right">
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild><Button variant="ghost" className="h-8 w-8 p-0"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger>
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => openDialog(doc)}>수정</DropdownMenuItem>
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                  <AlertDialogHeader><AlertDialogTitle>정말로 삭제하시겠습니까?</AlertDialogTitle><AlertDialogDescription>이 작업은 되돌릴 수 없습니다. {doc.name} 의료진의 정보가 서버에서 영구적으로 삭제됩니다.</AlertDialogDescription></AlertDialogHeader>
                                  <AlertDialogFooter><AlertDialogCancel>취소</AlertDialogCancel><AlertDialogAction onClick={() => handleDelete(doc.id)}>삭제</AlertDialogAction></AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </DropdownMenuContent>
                        </DropdownMenu>
        ...
        <DoctorDialog isOpen={isDialogOpen} setIsOpen={setIsDialogOpen} doctor={selectedDoctor} branches={branches} onSave={fetchDoctorsAndBranches}>
            {/* This component is now controlled by state, so it doesn't need a trigger child */}
        </DoctorDialog>
      </div>
    );
  }